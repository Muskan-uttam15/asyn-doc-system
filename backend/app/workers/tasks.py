...
Each task runs in the Celery worker process (sync context).
Progress events are published via Redis Pub/Sub so the FastAPI
...
def _parse_document(file_path: str, filename: str, mime_type: str | None) -> dict:
    """Extract real metadata + simulate text parsing."""
...
def _extract_fields(parsed: dict, raw_text: str = "") -> dict:
    ...
    # Simulate: derive title from filename
    title = os.path.splitext(filename)[0].replace("_", " ").replace("-", " ").title()

    # Simulate category based on extension
    category_map = {
        ".pdf": "PDF Document",
        ".txt": "Plain Text",
        ".md": "Markdown",
    ...
    # Simulate keywords from filename tokens
...
def retry_job(job_id: str):
    """Reset job state and re-dispatch processing."""
    session = get_sync_session()
    try:
        job: ProcessingJob = session.query(ProcessingJob).filter_by(id=uuid.UUID(job_id)).first()
        if not job:
